...
const hvSeoRenderModel = (pairs, ctx, target) => {
  ...
  document.addEventListener("click", (evt) => {
    ...
    if (url.searchParams.has("kt")) return;
  ...
  let recentVehiclesHeaderLoadPromise = null;
  ...
  function initRecentVehiclesHeader() {
    ...
    if (!recentVehiclesHeaderLoadPromise) {
      recentVehiclesHeaderLoadPromise = loadScriptOnce(
        "/assets/js/recentVehiclesHeader.js?v=20260901-1",
        "HVRecentVehiclesHeader"
      )
        .then(() => {
          if (
            window.HVRecentVehiclesHeader &&
            typeof window.HVRecentVehiclesHeader.init === "function"
          ) {
            window.HVRecentVehiclesHeader.init();
          }
        })
  ...
  window.addEventListener("hv:vehicleYearRange", (e) => {
    ...
    const elMin =
      document.getElementById("flt-year-from") ||
      document.querySelector("[name='flt-year-from'], [data-year-min]");
    const elMax =
      document.getElementById("flt-year-to") ||
      document.querySelector("[name='flt-year-to'], [data-year-max]");
    setYearInputs(elMin, elMax, FILTER.yearRange);
    updateFilterSummary();
  ...
  function setYearInputs(fromInput, toInput, range, fallback) {
  ...
  function renderModel(kits, makes, makeSlug, modelSlug) {
    ...
    if (FILTER.yearRange) {
      setYearInputs(yearFromInput, yearToInput, FILTER.yearRange);
    }
    if (plateContext && plateContext.yearRange) {
      setYearInputs(yearFromInput, yearToInput, plateContext.yearRange);
    }
    if (FILTER.drive.size) {
